defmodule Dbservice.BlendedLearningMentorshipMappingSchemaTest do
  ...
  test "defines the canonical Mapping contract and indexes" do
    ...
    assert indexes() == [
             {"blended_learning_mentor_mentee_mappings_pkey", true, ["id"], nil},
             {"blm_mappings_active_mentor_year_idx", false,
              ["mentor_user_id", "academic_year", "student_id"], "(ended_at IS NULL)"},
             {"blm_mappings_active_program_year_idx", false,
              ["program_id", "academic_year", "student_id"], "(ended_at IS NULL)"},
             {"blm_mappings_active_student_year_unique", true, ["student_id", "academic_year"],
              "(ended_at IS NULL)"},
             {"blm_mappings_student_history_idx", false,
              ["student_id", "academic_year", "started_at"], nil}
           ]
  end

  test "maps an operations mentor to a student without a school" do
    scope = insert_scope()

    assert {:ok, %{rows: [[mapping_id]]}} = insert_mapping(scope, [nil, nil, nil, nil])

    assert Repo.query!(
             "SELECT school_id, mentor_user_id, assignment_source FROM #{@table} WHERE id = $1",
             [mapping_id]
           ).rows == [[nil, scope.mentor_user_id, "af_lms"]]
  end
  ...
  test "stores audit snapshots and rejects blank or active end-event metadata" do
    ...
    Repo.query!(
      ...
      WHERE id = $3
      """,
      ["ops@example.com", assignment_reason, mapping_id]
    )

    assert Repo.query!(
             "SELECT assigned_by_email, assignment_audit_reason FROM #{@table} WHERE id = $1",
             [mapping_id]
           ).rows == [["ops@example.com", assignment_reason]]
    ...
    Repo.query!(
      ...
      WHERE id = $3
      """,
      ["ops@example.com", end_reason, mapping_id]
    )

    assert Repo.query!(
             "SELECT ended_by_email, end_audit_reason FROM #{@table} WHERE id = $1",
             [mapping_id]
           ).rows == [["ops@example.com", end_reason]]
  ...
  test "retains ended assignment history and restricts canonical deletion" do
    ...
    assert_unique_violation(fn ->
      insert_mapping(%{scope | mentor_user_id: other_mentor_user_id}, [nil, nil, nil, nil])
    end)
  ...
  defp insert_scope do
    ...
    [[product_id]] =
      Repo.query!(
        "INSERT INTO product (name, inserted_at, updated_at) VALUES ('BL Mapping', now(), now()) RETURNING id"
      ).rows

    [[program_id]] =
      Repo.query!(
        "INSERT INTO program (name, product_id, inserted_at, updated_at) VALUES ('BL Mapping', $1, now(), now()) RETURNING id",
        [product_id]
      ).rows
  ...
  defp insert_mapping(scope, [ended_at, ended_by_user_id, end_reason, end_source]) do
    Repo.query(
      """
      INSERT INTO #{@table}
        (student_id, mentor_user_id, program_id, academic_year, started_at,
         assignment_source, ended_at, ended_by_user_id, end_reason, end_source)
      VALUES ($1, $2, $3, '2026-2027', $4, 'af_lms', $5, $6, $7, $8)
      RETURNING id
      """,
      [
        scope.student_id,
